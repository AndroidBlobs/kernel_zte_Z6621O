...
static void chg_bq2560x_chgr_cur_limit(u32 limit)
{
	...
	if (limit > 3200)
		limit = 3200;
	else if (limit < REG00_IINLIM_BASE)
		limit = REG00_IINLIM_BASE;

	reg_value = (limit - REG00_IINLIM_BASE) / REG00_IINLIM_BASE;
	bq2560x_set_chg_current_limit(reg_value);
}

static void chg_bq25601x_set_highz_mode(u32 enable)
{
	pr_info("set highz=%d\n", enable);
	if (enable > 0)
...
static bool chg_bq25601x_get_highz_mode(void)
{
	u32 ret = 0;

	ret = bq2560x_get_highz_mode();
...
struct sprd_ext_ic_operations bq2560x_op = {
	...
	.set_ic_highz_mode = chg_bq25601x_set_highz_mode,
	.get_ic_highz_mode = chg_bq25601x_get_highz_mode,
};
