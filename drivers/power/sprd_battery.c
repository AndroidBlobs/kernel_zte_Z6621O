					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		if (sprd_ext_ic_op->get_ic_highz_mode) {
			val->intval = !sprd_ext_ic_op->get_ic_highz_mode();
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (val->intval == 0) {
			if (sprd_ext_ic_op->set_input_cur_limit == NULL)
				sprdbat_data->stop_charge();
			else if (sprd_ext_ic_op->set_ic_highz_mode)
				sprd_ext_ic_op->set_ic_highz_mode(1);
			else {
			...
			if (sprd_ext_ic_op->set_input_cur_limit == NULL)
				...
				sprd_ext_ic_op->set_ic_highz_mode(0);
			else {
...
static int sprdbat_start_charge(void)
{
	...
	if (sprd_ext_ic_op->get_ic_highz_mode)
		sprdbat_data->bat_info.charging_enabled = !sprd_ext_ic_op->get_ic_highz_mode();
	else
