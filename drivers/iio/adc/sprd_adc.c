...
static inline u32 sprd_glb_reg_read(u32 reg)
{
	u32 val = 0;
	int ret = 0;

	ret = regmap_read(pmic_adc->regmap, reg, &val);
	if (ret < 0)
		pr_err("%s: read failed ret=%d val=%d.\n", __func__, ret, val);
	return val;
}
...
static void sprd_adc_dump_register(void)
{
	...
	pr_info("m_en = 0x%x\n", sprd_glb_reg_read(ANA_REG_GLB_ARM_MODULE_EN));
	pr_info("clk_en = 0x%x\n", sprd_glb_reg_read(ANA_REG_GLB_ARM_CLK_EN));
	pr_info("xtl_en = 0x%x\n", sprd_glb_reg_read(ANA_REG_GLB_XTL_WAIT_CTRL));
