...
#define POLLING_DELAY_TIME 200000000   /*200ms*/
...
#ifndef INTERRPUT_MODE
	hrtimer_start(&ltr558_ptr->timer, ns_to_ktime(POLLING_DELAY_TIME), HRTIMER_MODE_REL);
#endif
...
#ifndef INTERRPUT_MODE
	hrtimer_cancel(&ltr558_ptr->timer);
#endif
...
static void ltr558_work(struct work_struct *work)
{
	...
	if ((0x01 == (status & 0x01)) && (LTR_PLS_MODE == LTR_PLS_558)) {/*is 558 PS*/
	...
	if ((0x01 == (status & 0x01)) && (LTR_PLS_MODE == LTR_PLS_553)) {/*is 553 PS*/
		value = ltr558_i2c_read_2_bytes(LTR558_PS_DATA_0);
...
{
	struct tag_ltr558 *ltr_558als =
	    container_of(timer, struct tag_ltr558, timer);
	queue_work(ltr_558als->ltr_work_queue, &ltr_558als->work);
	hrtimer_forward_now(&ltr_558als->timer, ns_to_ktime(POLLING_DELAY_TIME));
...
#else
		hrtimer_init(&ltr_558als->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
		ltr_558als->timer.function = ltr_timer_func;
#endif

	ltr558_ptr = ltr_558als;
...
#ifndef INTERRPUT_MODE
		hrtimer_try_to_cancel(&ltr_558als->timer);
#else
