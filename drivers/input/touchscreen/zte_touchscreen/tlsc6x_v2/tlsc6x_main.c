...
int tlsc6x_do_suspend(void)
{
	...
	if (gpio_is_valid(g_tp_drvdata->platform_data->power_gpio_number)) {
		gpio_direction_output(g_tp_drvdata->platform_data->power_gpio_number, 0);
		gpio_direction_output(g_tp_drvdata->platform_data->reset_gpio_number, 0);
	}
	g_tp_drvdata->suspended = true;
	return 0;
}
...
static void tlsc6x_resume_work(struct work_struct *work)
{
	...
	if (gpio_is_valid(g_tp_drvdata->platform_data->power_gpio_number)) {
		gpio_direction_output(g_tp_drvdata->platform_data->power_gpio_number, 1);
